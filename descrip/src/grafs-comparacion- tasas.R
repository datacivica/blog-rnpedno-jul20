...
tempo <- base %>% 
  ...
  summarise(tot = sum (cuenta, na.rm = T), pob = sum (pob), 
            tasa = (tot/pob)*100000) %>% 
  ungroup () %>% 
  distinct() %>% 
  select(-c(tot, pob)) %>% 
...
ggplot(tempo, aes(x =  factor(fuente, levels = c("CENAPI", "RNPEDNO")), 
  ...
  labs(title = " Tasa de personas desaparecidas registradas por fuente y entidad",
       ...
       x = "Fuente", y = "",
       caption = "Fuente: CENAPI y RNPDNO \n En los estados resaltados de rojo se registró mayor tasa de personas desaparecidas en el CENAPI que en el RNPDNO") +
  tema +
...
tempo <- base %>% 
 ...
 ggplot(tempo, aes(x = year, y = tasa, group = fuente, color = fuente)) +
  geom_line() +
  scale_color_viridis(discrete = TRUE, option = "cividis") +
   labs (title = "Tasa de personas desaparecidas registradas por fuente y año",
         subtitle = "Por cada 100,000 habitantes de 2000 a 2018",
         caption = "Fuente: CENAPI y RNPDNO") +
  tema
 ...
 ggplot(tempo, aes(x = year, y = tasa, group = fuente, color = fuente)) +
   ...
   facet_wrap(~sexo) +
   scale_color_viridis(discrete = TRUE, option = "cividis") +
   labs (title = "Tasa de personas desaparecidas registradas por sexo fuente y año",
         subtitle = "Por cada 100,000 habitantes de 2000 a 2018",
         caption = "Fuente: CENAPI y RNPDNO") +
   tema
