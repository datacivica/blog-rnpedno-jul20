...
clean_cenapi <- rename(df_cenapi,
  ...
  mutate(across(c("fecha_evento", "fecha_localizacion"),
                str_trim, side = "both")) %>% 
  separate(fecha_evento, into = c("year_evento", "mes_evento", "dia_evento"),
           sep = "-", remove = F) %>% 
  separate(fecha_localizacion, into = c("year_loc", "mes_loc", "dia_loc"),
           sep = "-", remove = F) %>% 
  mutate(inegi_evento = paste0(clave_estado, clave_municipio),
         ...
         vivo_muerto = case_when(vivo_muerto == "AUN SIN LOCALIZAR" ~ "Aun sin localizar",
  ...
  select(vivo_muerto, fecha_evento:year_evento, clave_estado, clave_municipio, 
         ...
         fecha_localizacion:year_loc)


# Cruzamos con nombres

clean_cenapi <- left_join(clean_cenapi, df_names, by = c("clave_estado" = "cve_ent")) %>%
  rename(nom_ent_evento = nom_ent) %>%
  left_join(., df_names, by = c("clave_estado_localizado" = "cve_ent")) %>%
  rename(nom_ent_localizado = nom_ent) %>% 
  select(vivo_muerto, year_evento, clave_estado,
         nom_ent_evento, clave_municipio, inegi_evento,
         sexo:grupo_edad, clave_estado_localizado,
         nom_ent_localizado, clave_municipio_localizado,
         inegi_localizado, year_loc)

saveRDS(clean_cenapi, files$out_cenapi)
